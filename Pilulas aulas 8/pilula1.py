def ranking_prioridade(pacientes):
    ...
    for paciente in pacientes:
        ...
        ranking.append({'nome': paciente, 'pontos': pontos})
    ...
    for i in range(len(ranking)):
        print(f"{i+1} - {ranking[i]['nome']['nome']}, {ranking[i]['pontos']}")

def main():
    pacientes = [
        {'nome': 'Ana', 'idade': 70, 'gravidade': 3},
        {'nome': 'Carlos', 'idade': 30, 'gravidade': 5},
        {'nome': 'Beatriz', 'idade': 5, 'gravidade': 2},
